...
void TestFieldAssignment() {
    ...
    ASSERT(subobject != nullptr &&
           subobject->Fields().find("z"s) != subobject->Fields().end());
...
void TestBadAddition() {
    ...
    ASSERT_THROWS(Add(make_unique<NumericConst>(42), make_unique<StringConst>("4"s))
                      .Execute(empty, context),
                  std::runtime_error);
    ASSERT_THROWS(Add(make_unique<StringConst>("4"s), make_unique<NumericConst>(42))
                      .Execute(empty, context),
                  std::runtime_error);
    ASSERT_THROWS(
        Add(make_unique<None>(), make_unique<StringConst>("4"s)).Execute(empty, context),
        std::runtime_error);
...
void TestBaseClass() {
    vector<runtime::Method> methods;
    methods.push_back(
        {"GetValue"s, {}, make_unique<VariableValue>(vector{"self"s, "value"s})});
